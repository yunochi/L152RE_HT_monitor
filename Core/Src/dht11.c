...
const THdata DHT11_SENSOR_ERROR = {0xFFFF,0xFFFF};
...
THdata dht11_getSensorData(){
    ...
    //센서 응답 대기
    while(HAL_GPIO_ReadPin(TH_pin_GPIO_Port,TH_pin_Pin) != GPIO_PIN_RESET){if(HAL_GetTick() - startTime > 1000){return DHT11_SENSOR_ERROR;}}
    while(HAL_GPIO_ReadPin(TH_pin_GPIO_Port,TH_pin_Pin) != GPIO_PIN_SET){if(HAL_GetTick() - startTime > 1000){return DHT11_SENSOR_ERROR;}}
    ...
    sensorVal.rh = ((rawRxData >> 24)*10) + ((rawRxData >> 16) & 0xFF);
    //sensorVal.rh = rawRxData >>16;
    sensorVal.temp = (int32_t)(((rawRxData>>8) & 0xFF)*10) + (rawRxData & 0xFF);
